...
const samplePredictionData = {
  ...
  compliance: [
    { month: "Jan", actual: 75, predicted: 78 },
    { month: "Feb", actual: 82, predicted: 80 },
    { month: "Mar", actual: 88, predicted: 86 },
    { month: "Apr", actual: 85, predicted: 84 },
    { month: "May", actual: 90, predicted: 88 },
    { month: "Jun", actual: 92, predicted: 90 },
    { month: "Jul", actual: null, predicted: 94 },
    { month: "Aug", actual: null, predicted: 96 },
    { month: "Sep", actual: null, predicted: 98 },
  ],
}

export function PredictiveAnalytics({ type = "enrollment" }) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      setData(samplePredictionData[predictionType])
      setLoading(false)
    }
  ...
  const insights = {
    enrollment: {
      key: "Enrollment is predicted to increase by 12% in the next quarter based on current trends.",
      secondary: "Seasonal patterns suggest higher enrollment in August-September.",
      recommendation: "Prepare additional resources for the predicted enrollment spike.",
    },
    completion: {
      key: "Completion rates are expected to improve by 8% with the introduction of new microlearning modules.",
      secondary: "Courses with interactive elements show 15% higher predicted completion rates.",
      recommendation: "Focus on enhancing course interactivity to boost completion rates.",
    },
    engagement: {
      key: "User engagement is projected to grow steadily with peak activity expected in August.",
      secondary: "Content engagement is predicted to be highest for video-based materials.",
      recommendation: "Increase video content production to capitalize on engagement trends.",
    },
    compliance: {
      key: "Compliance rates are predicted to reach 98% by the end of Q3.",
      secondary: "Automated reminders have shown to improve compliance rates by 12%.",
      recommendation: "Implement automated reminder system for upcoming compliance deadlines.",
    },
  }
      ...
      <CardContent>
        <Tabs defaultValue="chart" value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="chart" className="w-full">
            {loading ? (
              <div className="h-[350px] w-full bg-muted/50 animate-pulse rounded-md flex items-center justify-center">
              ...
              <div className="rounded-lg bg-muted p-4">
                ...
                <ul className="space-y-2 text-sm">
                  <li className="flex items-start gap-2">
                    ...
                    <span>{insights[predictionType].key}</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="mt-0.5 rounded-full bg-blue-500 p-1"></span>
                    <span>{insights[predictionType].secondary}</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="mt-0.5 rounded-full bg-yellow-500 p-1"></span>
                    <span>{insights[predictionType].recommendation}</span>
                  </li>
                </ul>
              </div>
              <div className="rounded-lg bg-muted p-4">
